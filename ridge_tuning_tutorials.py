...
def bayes_objective(trial):
    params = {
        'ridge__alpha': trial.suggest_float('ridge__alpha', 0.0001, 100, log=True),
    }
    # モデルにパラメータ適用
...
# 検証曲線描画対象パラメータ
valid_curve_params = {'ridge__alpha': [0.00001, 0.0001, 0.001, 0.003, 0.01, 0.03, 0.1, 0.3, 1, 3, 10, 100]
                      }
param_scales = {'ridge__alpha': 'log'
                }
# 最適パラメータを上記描画対象に追加
...
trained_model = Pipeline([('scaler', StandardScaler()), ('ridge', Ridge())])
trained_model.set_params(**best_params)
trained_model.fit(X_train, y_train)
coef = trained_model['ridge'].coef_
intercept = trained_model['ridge'].intercept_
